...
import { readableStreamToString, BlueTimeError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new BlueTimeError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new BlueTimeError(e, e.response?.data)
                }
                if (innerError instanceof BlueTimeError) {
                    // Got "BlueTimeError" from the above try block
                    throw innerError;
                }
